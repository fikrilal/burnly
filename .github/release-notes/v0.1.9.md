...
Improves tray model visibility and refreshes Burnly's launcher branding.

## Highlights

- Shows all models with usage today in the tray panel instead of collapsing
  lower-ranked rows into a generic `Other` bucket.
- Keeps source-level fallback labels only where the upstream data cannot provide
  per-model splits.
- Replaces the app icon with the new orange Burnly mark across Linux, Windows,
  and macOS packaged assets.
- Includes Linux launcher icon installation hardening so desktop environments
  can resolve the installed icon reliably.
...
```bash
curl -fsSL https://github.com/fikrilal/burnly/releases/download/v0.1.9/install-linux.sh | BURNLY_VERSION=v0.1.9 sh
```
...
```text
burnly-v0.1.9-windows-x86_64.exe
```
...
```bash
curl -fsSL https://github.com/fikrilal/burnly/releases/download/v0.1.9/install-macos.sh | BURNLY_VERSION=v0.1.9 sh
```

Manual `.dmg` installation is also available:

```text
burnly-v0.1.9-macos-aarch64.dmg
burnly-v0.1.9-macos-x86_64.dmg
```
