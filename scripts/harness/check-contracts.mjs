...
if (!/const CONTRACT_VERSION: u16 = 2;/.test(responseSource)) {
  failures.push("Rust IPC contract version must remain explicitly set to 2.");
}
...
async function checkResponseFixtures() {
  ...
  for (const fixtureName of fixtureNames) {
    ...
    if (fixture.meta?.contractVersion !== 2) {
      failures.push(`${fixtureName}: contractVersion must be 2.`);
    }
