...
1. **Source-reported** (`source_reported`): the tool's own computed cost
   (Command Code `costUsd`, Cline cost metrics).
2. **Collector-calculated** (`collector_calculated`): ccusage's offline
   estimate for Claude Code, Codex, OpenCode, and Pi.
...
| OpenCode     | Supported         | Collected through Burnly's native local SQLite collector across legacy and OpenCode 2 schemas under one source identity. Reads usage-only fields, deduplicates overlapping stable message IDs with OpenCode 2 precedence, and preserves provider-qualified model breakdowns.                                                                                                                                                                                                                                                                                                                          |
