...
Antigravity, Grok Build, and Command Code, and gives developers a compact view
...
Cost has four provenances, in precedence order:

1. **Source-reported** (`source_reported`): the tool's own computed cost
   (OpenCode session/message cost, Command Code `costUsd`, Cline cost metrics).
2. **Collector-calculated** (`collector_calculated`): ccusage's offline
   estimate for Claude Code, Codex, and Pi.
3. **Burnly-calculated** (`burnly_calculated`): priced by Burnly from an
   embedded models.dev pricing snapshot for sources that report no cost
   (Grok, Antigravity, ZCode).
4. **Gap-fill**: when a source reports zero cost with positive tokens and
   the embedded snapshot prices the model, Burnly fills the cost as
   `burnly_calculated`. This never overrides a positive reported or
   calculated cost.

All cost is an estimate (models.dev is community-maintained), not a bill.
...
| OpenCode     | Supported         | Collected through Burnly's native read-only SQLite collector across legacy and preview OpenCode 2 schemas under one source identity. It retains legacy-only history, gives V2 precedence to overlapping stable IDs, preserves provider-qualified model breakdowns and all token categories, uses source-reported estimated cost, and recovers compacted cumulative remainder as partial `OpenCode unattributed` usage. Prompt, response, tool, title, project, and account fields are not selected. OpenCode 2 schema compatibility may require updates as the preview evolves.                       |
...
| Grok Build   | Experimental      | Collected through Burnly's native local collector. Reads `shell.turn.inference_done` rows from `~/.grok/logs/unified.jsonl` and joins session metadata from `~/.grok/sessions/**/summary.json`. Totals count each model inference call, not each user turn. Cached prompt tokens count toward tray totals; `cache_read_tokens` is breakdown metadata only. Cost is Burnly-calculated from an embedded models.dev pricing snapshot. Grok formats are reverse-engineered and may change upstream. Burnly never reads chat transcripts, ACP updates, prompt history, terminal logs, or auth credentials. |
| Command Code | Experimental      | Collected through Burnly's native local collector. Reads per-message `usage` blocks from `~/.commandcode/projects/**/<session>.jsonl` transcripts (session `version: 3`). Totals sum provider-reported `inputTokens`, `outputTokens`, `cacheReadTokens`, and `cacheWriteTokens` per assistant message; cost is provider-computed `costUsd`. Legacy pre-1.11 transcripts carry no usage and are skipped. Command Code formats are reverse-engineered and may change upstream. Burnly never reads prompt/tool content, checkpoints, history, or auth credentials.                                       |
| Zed          | Experimental      | Collected through Burnly's native local collector. Reads agent thread token usage from `~/.local/share/zed/threads/threads.db` (zstd-compressed thread JSON); one session per thread, daily usage attributed to the thread's local day. Cost is Burnly-calculated from an embedded models.dev pricing snapshot with the `zed.dev/` provider prefix normalized away. Per-request history in the Zed telemetry log is a staged cross-check. Zed formats are reverse-engineered and may change upstream. Burnly never deserializes thread message content, prompts, responses, or tool inputs/outputs.   |
