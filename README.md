...
| Zed          | Experimental      | Native Burnly collector for `~/.local/share/zed` | Reads agent thread token usage from `threads.db` (zstd thread JSON). Cost is Burnly-calculated from an embedded models.dev pricing snapshot. Per-request history in the telemetry log is a staged cross-check. Data format may change upstream.                                              |
