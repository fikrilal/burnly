...
use crate::application::usage::{
    ...
    TraySummaryReadModel, TraySummaryTrend, TraySummaryTrendDirection,
};
...
mod tests {
    ...
    #[test]
    fn tray_summary_response_uses_compact_contract() {
        let response = TraySummaryResponse::try_from(TraySummaryReadModel {
            ...
            week: TraySummaryPeriodMetric {
                ...
                total_tokens: 183_240,
            },
            month: TraySummaryPeriodMetric {
                start_date: date(2026, 6, 1),
                end_date: date(2026, 6, 30),
                total_tokens: 612_900,
            },
            models: vec![TraySummaryModelRow {
                model_name: "GPT-5.1".to_owned(),
                agent_label: "Codex".to_owned(),
                total_tokens: 25_000,
                trend: Some(TraySummaryTrend {
                    direction: TraySummaryTrendDirection::Increased,
                    basis_points: 850,
                }),
            }],
            as_of_ms: 1_782_375_600_000,
            last_successful_refresh_at_ms: Some(1_782_375_300_000),
            data_status: OverviewDataStatus::Current,
        })
        .expect("tray summary response");

        let value = to_value(response).expect("serialize response");

        assert_eq!(value["today"]["totalTokens"], "42180");
        assert_eq!(value["models"][0]["modelName"], "GPT-5.1");
        assert_eq!(value["models"][0]["agentLabel"], "Codex");
        assert_eq!(value["models"][0]["trend"]["direction"], "increased");
        assert_eq!(value["models"][0]["trend"]["basisPoints"], 850);
